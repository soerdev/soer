...
import { PersonalTarget, RoadmapTask } from 'src/app/api/targets/target.interface';
...
export class RoadmapComponent implements OnInit {
  ...
  private calcProgress(tasks: RoadmapTask[]) {
      if ((tasks ?? []).length > 0) {
        let value = tasks.reduce( (r, v) => ({total: r.total + 100, real: r.real + v.progress}), {total: 0, real: 0});
